...
export class PlanService {
  ...
  private parseIOTimings(text: string, el: Node): boolean {
    ...
    // 1. Handle scoped entries like "local read=10 write=20"
    ...
    const scopeIsHalfDetailed = _.some(results, (result) => {
    ...
    if (scopeIsHalfDetailed || scopeIsDetailed) {
  ...
  private calculateExclusives(node: Node) {
    ...
    _.each(properties, (property) => {
      const sum = Number(_.sumBy(node[NodeProp.PLANS], (child: Node) => {
  ...
  private calculateIoTimingsAverage(node: Node) {
    ...
    _.forEach(scopesMatrix, (buffersScopes, timingScope) => {
      operations.forEach((operation, index) => {
        ["exclusive_", ""].forEach((prefix) => {
          const timeProp = `${prefix}${timingScope ? timingScope + "_": ""}io_${operation}_time`.toUpperCase() as keyof typeof NodeProp;
          const speedProp = `${prefix}average_${timingScope ? timingScope + "_": ""}io_${operation}_speed`.toUpperCase() as keyof typeof NodeProp;
          ...
          const buffers = _.sumBy(buffersScopes, (bufferScope) => {
            const bufferProp = `${prefix}${bufferScope}_${buffersOperation}_blocks`.toUpperCase() as keyof typeof NodeProp;
            return node[NodeProp[bufferProp]] as number || 0
          })
          if (time) {
            node[NodeProp[speedProp] as unknown as keyof typeof Node] = Number((buffers / (time / 1000)).toFixed(3))
          }
        })
      })
    })
    ...
    operations.forEach((operation, index) => {
      ["exclusive_", ""].forEach((prefix) => {
        const sumTimeProp = `${prefix}sum_io_${operation}_time`.toUpperCase() as keyof typeof NodeProp;
        const speedProp = `${prefix}average_sum_io_${operation}_speed`.toUpperCase() as keyof typeof NodeProp;
        let time = 0;
        let buffers = 0;
        _.forEach(scopesMatrix, (buffersScopes, timingScope) => {
          const timeProp = `${prefix}${timingScope ? timingScope + "_": ""}io_${operation}_time`.toUpperCase() as keyof typeof NodeProp;
