...
`
  ...
  it("correctly parses workers", () => {
    ...
    expect(root?.Plans[0].Workers?.length).toEqual(2)
    expect(root?.Plans[0].Workers?.[0]["Actual Startup Time"]).toEqual(1431.136)
    expect(root?.Plans[0].Workers?.[0]["Sort Method"]).toBeDefined()
  })
})
