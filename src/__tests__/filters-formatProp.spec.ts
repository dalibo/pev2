...
describe("formatProp", () => {
  ...
  describe("factor", () => {
    it("formats a factor with a times symbol", () => {
      expect(formatProp(Property.PLANNER_ESTIMATE_FACTOR, 3.456)).toBe(
        "3.5&nbsp;&times;",
      )
    })

    it("rounds to 2 significant figures", () => {
      expect(formatProp(Property.PLANNER_ESTIMATE_FACTOR, 1234)).toBe(
        "1,200&nbsp;&times;",
      )
    })

    it("returns N/A for undefined", () => {
      expect(formatProp(Property.PLANNER_ESTIMATE_FACTOR, undefined)).toBe(
        "N/A",
      )
    })
  })
  ...
  describe("blockAsBytes", () => {
    it("formats blocks", () => {
      expect(formatProp(Property.SHARED_HIT_BLOCKS, 1234)).toBe(
        "1,234<br><small>9.6 MB</small>",
      )
    })
  })
