...
test.describe("Simple join", () => {
  test.beforeEach(async ({ page }) => {
    await loadPlan(page, "Simple join (JSON format)")
  })
  ...
  test.describe("node", () => {
    test("node box toggle shows and hides tabs", async ({ page }) => {
      ...
      await expect(
        node.getByRole("link", { name: "IO & Buffers" }),
  ...
  test.describe("plan style", () => {
    ...
    test("displays plan with duration", async () => {
      const durationButton = rightPanel.getByRole("button", {
        name: "duration",
      })
...
test.describe("Parallel (4 workers)", () => {
  test.beforeEach(async ({ page }) => {
    await loadPlan(page, "Parallel (4 workers)")
  })
  test("approximative values for rows", async ({ page }) => {
    const node = page.locator(".plan-node", { hasText: "Parallel Hash Join" })
    const toggle = node.getByRole("heading", { name: "Parallel Hash Join" })
